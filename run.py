...
def push_to_approved_sheet(invoice_date, item_code, site, invoice_price, system_price, document_reference, buyer, price_from_lpf):
    ...
    row = [invoice_date, document_reference, item_code, site, invoice_price, system_price, buyer, price_from_lpf, status]
...
def create_rejected_invoices_report():
    while True:
        ...
        price, buyer, price_from_lpf = query_item_price_and_buyer(invoice_date, item_code, site)

        print("\n=== Report Results ===")
        if price is not None:
            if abs(float(invoice_price) - float(price)) <= 0.01:
                push_to_approved_sheet(invoice_date, item_code, site, invoice_price, system_price, document_reference, buyer, price_from_lpf)
                ...
                push_to_rejected_sheet(invoice_date, item_code, site, invoice_price, system_price, document_reference, buyer, price_from_lpf)
